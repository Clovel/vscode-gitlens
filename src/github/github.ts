...
export class GitHubApi implements Disposable {
	...
	private _graphql: typeof import('@octokit/graphql').graphql | undefined;
	private async graphql(): Promise<typeof import('@octokit/graphql').graphql> {
		if (this._graphql === undefined) {
			this._graphql = (await import('@octokit/graphql')).graphql;
		}
